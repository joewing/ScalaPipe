...
private[scalapipe] trait IRNode {
...
private[scalapipe] case class IRStart(
        val next:    Int = -1,
...
private[scalapipe] case class IRNoOp(
        val id: Int = IRNodeCounter.getId
    ) extends IRNode {
...
private[scalapipe] case class IRInstruction(
...
private[scalapipe] case class IRStore(
        val dest:   BaseSymbol,
...
private[scalapipe] case class IRLoad(
        val dest:   BaseSymbol,
        val src:    BaseSymbol,
        val offset: BaseSymbol,
        val id:     Int = IRNodeCounter.getId
...
private[scalapipe] case class IRGoto(
        val next:    Int = -1,
        val id:      Int = IRNodeCounter.getId
    ) extends IRNode {
...
private[scalapipe] case class IRStop(
        val id: Int = IRNodeCounter.getId
    ) extends IRNode {
...
private[scalapipe] case class IRReturn(
...
private[scalapipe] case class IRConditional(
        val test:    BaseSymbol,
        val iTrue:  Int = -1,
        val iFalse: Int = -1
    ) extends IRNode {
...
private[scalapipe] case class IRSwitch(
        val test: BaseSymbol,
...
private[scalapipe] case class IRCall(
        val func: String,
...
private[scalapipe] case class IRPhi(
